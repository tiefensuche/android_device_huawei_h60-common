...
int usage(void)
{
    fprintf(stderr,"usage: mkbootimg-h60\n"
            "       --secureboot_area <size>\n"
            "       --kernel <filename>\n"
            "       --ramdisk <filename>\n"
...
int write_secureboot_signature(int fd, unsigned itemsize)
{
    unsigned count;
...
int main(int argc, char **argv)
{
    ...
    while(argc > 0){
        ...
        if(!strcmp(arg, "--output") || !strcmp(arg, "-o")) {
            bootimg = val;
        } else if(!strcmp(arg, "--secureboot_area")) {
            secureboot_area_size = strtoul(val, 0, 16);
    ...
    if(secureboot_area_size > 0) {
        if(write_secureboot_signature(fd, secureboot_area_size)) goto fail;
    }
