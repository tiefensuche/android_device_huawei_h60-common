...
int usage() {
    printf("usage: unpackbootimg-h60\n");
...
int main(int argc, char** argv)
{
    ...
    for (magic_addr = 0; magic_addr <= 4096; magic_addr++) {
        fseek(f, magic_addr, SEEK_SET);
        fread(tmp, BOOT_MAGIC_SIZE, 1, f);
        if (memcmp(tmp, BOOT_MAGIC, BOOT_MAGIC_SIZE) == 0)
            break;
    }
    total_read = magic_addr;
    ...
    if (magic_addr > 0) {
        ...
        FILE *sb = fopen(tmp, "wb");
        byte* secureboot = (byte*)malloc(secureboot_area_size);
        fread(secureboot, secureboot_area_size, 1, f);
        fwrite(secureboot, secureboot_area_size, 1, sb);
        fclose(sb);
    }
